    ...
    async function loadData() {
      try {
        ...
        const [dash, pools, logs] = await Promise.all([
          ...
          api.getAuditLogs() as Promise<DashboardAuditLog[]>,
        ]);
                ...
                {auditLogs.filter(log => ['MANUAL_TRANSACTION', 'JOIN_POOL', 'CONFIRM_ORDER'].includes(log.action)).length === 0 ? (
                  ...
                  auditLogs.map((log, index) => {
                    ...
                    const quantityText = isManual 
                      ? `+${(detailNumber(details, 'quantity') / 1000).toFixed(1)} Ton`
                      : (isJoin ? `+${(detailNumber(details, 'quantity') / 1000).toFixed(1)} Ton` : `+${(detailNumber(details, 'totalVolume') / 1000).toFixed(1)} Ton`);
                    
                    const fertilizerName = isManual 
                      ? detailText(details, 'jenisPupuk', 'Pupuk')
                      : (isJoin ? detailText(details, 'jenisPupuk', 'NPK Phonska') : 'Pupuk NPK Phonska');

                    const supplierName = isManual 
                      ? detailText(details, 'supplierName', 'Pemasok')
